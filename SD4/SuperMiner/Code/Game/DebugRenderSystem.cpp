...
void DebugRenderSystem::AddCurrent3DTask()
{
	...
	case RENDER_TASK_BASIS:
		MakeDebugRenderBasis(m_defaultLifetime, drawPos, 1.f, RIGHT, UP, FORWARD, m_currentMode);
		break;
...
void DebugRenderSystem::MakeDebugRenderBasis(float lifetime, Vector3 position, float size, Matrix44 const & basis, DebugRenderMode const mode)
{
	DebugRenderTask_Basis* point = new DebugRenderTask_Basis(position, basis, size, lifetime, mode);
	m_tasks3D.push_back((DebugRenderTask*) point);
}

void DebugRenderSystem::MakeDebugRenderBasis(float lifetime, Vector3 position, float size, Vector3 right, Vector3 up, Vector3 forward, DebugRenderMode const mode)
{
	DebugRenderTask_Basis* point = new DebugRenderTask_Basis(position, right, up, forward, size, lifetime, mode);
	m_tasks3D.push_back((DebugRenderTask*) point);
}
