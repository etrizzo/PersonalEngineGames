...
GameState_Playing::GameState_Playing()
{
	...
	for (int i = 0; i < 28; i++)
	{
		Entity* ent = new Entity(Vector2(1.75f + (i * .5f), 7.f), Vector2::HALF, "white", RGBA::GetRandomEarthTone());
	...
	for (int i = 0; i < 28; i++)
	{
		Entity* ent = new Entity(Vector2(1.75f + (i * .5f), 6.f), Vector2::HALF, "white", RGBA::GetRGBAFromHSV(GetRandomFloatZeroToOne(), .5f, .75f));
		m_scene->AddRenderable(ent->m_renderable);
	}
	...
	for (int i = 0; i < 28; i++)
	{
		...
		Entity* ent = new Entity(Vector2(1.75f + (i * .5f), 5.f), Vector2::HALF , "white", entColor);
		m_scene->AddRenderable(ent->m_renderable);
	}
	...
	for (int i = 0; i < 28; i++)
	{
		Entity* ent = new Entity(Vector2(1.75f + (i * .5f), 4.f), Vector2::HALF , "white", RGBA::GetGoldenRatioColorSequence(.5f, .75f));
		m_scene->AddRenderable(ent->m_renderable);
	}
	...
	for (int i = 0; i < 28; i++)
	{
		Entity* ent = new Entity(Vector2(1.75f + (i * .5f), 3.f), Vector2::HALF, "white", RGBA::GetRandomMixedColor(RGBA(149,200,45), .8f));
		m_scene->AddRenderable(ent->m_renderable);
	}
	...
	{
		float perc = (float) i / 28.f;
		Entity* ent = new Entity(Vector2(1.75f + (i * .5f), 2.f), Vector2::HALF, "white", RGBA::GetRGBAFromHSV(perc, 1.f, 1.f));
		m_scene->AddRenderable(ent->m_renderable);
	}
