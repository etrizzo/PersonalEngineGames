...
{
   return sampleColor.a - center.a > .8;
}
...
void main() 
{
   ...
   vec4 edgeColor = vec4 (0.f);
   int halfSize = 1;
   vec4 diffuseColor = texture(gTexDiffuse, passUV);
   for(int x = (-1 * halfSize); x < halfSize+1; x++ )
   {
      for(int y = (-1 * halfSize); y < halfSize+1; y++ )
      {
         vec2 newUVs = passUV;
         newUVs.x += (x * TEXEL_SIZE_X);
         newUVs.y += (y * TEXEL_SIZE_Y);
         newUVs = clamp(newUVs, 0.0, 1.0);
   ...
   outColor = (diffuse * passColor) + edgeColor ;
   //outColor = edgeColor;
}