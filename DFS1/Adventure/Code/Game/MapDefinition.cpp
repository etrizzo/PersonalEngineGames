...
MapDefinition::MapDefinition(tinyxml2::XMLElement* mapDefElement)
{
	...
	std::string musicPath			= ParseXmlAttribute(*mapDefElement, "soundtrack", std::string());

	m_mapMusic = g_theAudio->CreateOrGetSound("Data/Audio/" + musicPath);
	
	m_defaultTile = TileDefinition::GetTileDefinition(defTileName);
	MakeGenerationSteps(mapDefElement->FirstChildElement("GenerationSteps"));
}

MapDefinition * MapDefinition::GetMapDefinition(std::string definitionName)
{
	auto containsDef = s_definitions.find((std::string)definitionName); 
	MapDefinition* mapDef = nullptr;
	if (containsDef != s_definitions.end()){
		mapDef = containsDef->second;
	}
	return mapDef;
}
