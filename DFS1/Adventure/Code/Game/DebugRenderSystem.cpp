...
void DebugRenderSystem::AddCurrent3DTask()
{
	...
	if (m_camera != nullptr){
		...
		drawPos = g_theGame->m_camera->GetPosition() + (g_theGame->m_camera->GetForward() * 5.f);
	}
...
void DebugRenderSystem::MakeDebugRenderCircle(float lifetime, Disc2 const & circle, bool dottedLine, DebugRenderMode mode, RGBA const & start_color, RGBA const & end_color)
{
	DebugRenderTask_Circle* circ = new DebugRenderTask_Circle(circle, dottedLine, lifetime, start_color, end_color);
	circ->m_info.m_mode = mode;
	m_tasks2D.push_back((DebugRenderTask*) circ);
}

void DebugRenderSystem::MakeDebugRenderCircle(float lifetime, const Vector2 & center, float radius, bool dottedLine, DebugRenderMode mode, RGBA const & start_color, RGBA const & end_color)
{
	Disc2 disc = Disc2(center, radius);
	MakeDebugRenderCircle(lifetime, disc, dottedLine, mode, start_color, end_color);
}
