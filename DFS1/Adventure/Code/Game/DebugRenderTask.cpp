...
{
	RGBA color = Interpolate(m_info.m_startColor, m_info.m_endColor, m_info.m_age / m_info.m_lifetime);
	RGBA right = Interpolate(color, RGBA::RED, .5f);
	RGBA up = Interpolate(color, RGBA::GREEN, .5f);
	RGBA forward = Interpolate(color, RGBA::BLUE, .5f);

	g_theRenderer->DrawPoint(m_position, .1f, RIGHT, UP, FORWARD, right, up, forward);
}
...
DebugRenderTask_Circle::DebugRenderTask_Circle(Disc2 disc, bool dotted, float lifetime, RGBA startColor, RGBA endColor, DebugRenderMode mode)
	:DebugRenderTask(lifetime, startColor, endColor, mode)
{
	m_disc = disc;
	m_dotted = dotted;
...
void DebugRenderTask_Circle::Render()
{
	RGBA tint = Interpolate(m_info.m_startColor, m_info.m_endColor, m_info.m_age / m_info.m_lifetime);
	g_theRenderer->DrawRegularPolygon2(20, m_disc.center, m_disc.radius, 0.f, tint, m_dotted);
}
