...
void Texture::PopulateFromImage(const Image * image)
{
...
{
	auto containsShader = s_loadedTextures.find((std::string)texPath); 
	Texture* loadedTex = nullptr;
	if (containsShader != s_loadedTextures.end()){
		loadedTex = containsShader->second;
	} else {
		std::string shaderPath = directory + texPath;
		loadedTex = new Texture(shaderPath.c_str());
